한글 ANSI Encoding(cp-949) 지원 버전
===========

이 버전은 'dkovar'의 'analyzeMFT' 레포지토리를 포크하여 수정한 버전입니다.
이 도구는 NTFS의 MFT를 분석할 때 너무나도 유용한 오픈소스 도구이지만, 결과로 나오는 한글이 깨진다는 단점이 있었습니다. 본 레포지토리의 소스코드는 해당 이슈를 해결하였습니다.

다만, 인코딩이 ANSI의 'cp-949'인 경우만 해결하였으며, 다른 인코딩은 역시나 깨질 수 있습니다. 그 경우엔 원본과 현재 버전이 다른 부분을 참고하여 수정해 사용하시면 됩니다.
